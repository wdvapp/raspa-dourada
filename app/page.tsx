...
export default function Home() {
  ...
  useEffect(() => {
      ...
      if (view === 'GAME') { bgAudioRef.current.play().catch(() => {}); } 
      else { bgAudioRef.current.pause(); bgAudioRef.current.currentTime = 0; }
  }, [view]);
  ...
  const handleEnterGame = (game: Game) => {
    ...
    setActiveGame(game); setView('GAME'); setTimeout(() => { playRound(game); }, 50);
  };
  ...
  const playRound = async (gameOverride?: Game) => {
    ...
    for (const prize of currentPrizes) { cumulativeChance += (Number(prize.chance) || 0); if (random <= cumulativeChance) { winningPrize = prize; break; } }
    let finalGrid: Prize[] = [];
    if (winningPrize) {
        ...
        for (let i = 0; i < 6; i++) { if (otherOptions.length > 0) { const randomFiller = otherOptions[Math.floor(Math.random() * otherOptions.length)]; if (finalGrid.filter(p => p.name === randomFiller.name).length < 2) { finalGrid.push(randomFiller); } else { finalGrid.push(otherOptions.find(p => p.name !== randomFiller.name) || randomFiller); } } }
    } else {
        let attempts = 0; while (finalGrid.length < 9 && attempts < 100) { const randomPrize = currentPrizes[Math.floor(Math.random() * currentPrizes.length)]; const count = finalGrid.filter(p => p.name === randomPrize.name).length; if (count < 2) { finalGrid.push(randomPrize); } attempts++; }
        while (finalGrid.length < 9) { finalGrid.push(currentPrizes[0]); }
    }
    setPrizesGrid(finalGrid.sort(() => Math.random() - 0.5)); setLoading(false); setGameId(prev => prev + 1);
  };
  ...
  const checkWinner = () => {
    ...
    for (const [name, indices] of Object.entries(counts)) { if (indices.length >= 3 && !badItems.includes(name)) { const prizeObj = prizesGrid.find(p => p.name === name); return { indices: indices, amount: prizeObj && prizeObj.value > 0 ? formatCurrency(prizeObj.value) : name }; } }
    return null;
  };

  const handleGameFinish = () => {
    if (isGameFinished) return; setIsGameFinished(true); const result = checkWinner();
    if (result) { setResultType('WIN'); setWinningIndices(result.indices); setWinAmount(result.amount); triggerWin(); if (user) { const prizeValue = prizesGrid[result.indices[0]].value; updateDoc(doc(db, 'users', user.uid), { balance: increment(prizeValue) }); } } else { setResultType('LOSS'); }
    setTimeout(() => { setShowPopup(true); }, 500);
  };

 const triggerWin = () => { if (winAudioRef.current) { winAudioRef.current.currentTime = 0; winAudioRef.current.play().catch(() => {}); } confetti({ particleCount: 150, spread: 100, origin: { y: 0.6 }, colors: ['#FFD700', '#DAA520', '#FFFFFF'], gravity: 1.5, ticks: 200, scalar: 1.2 }); };
  ...
  const handleInstallApp = async () => { if (deferredPrompt) { deferredPrompt.prompt(); const { outcome } = await deferredPrompt.userChoice; if (outcome === 'accepted') { setDeferredPrompt(null); } } };
  ...
  return (
    <>
      <div className={`fixed top-4 left-1/2 -translate-x-1/2 z-[100] w-[90%] max-w-sm bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl p-4 flex items-center gap-4 transition-all duration-500 ${toast.visible ? 'translate-y-0 opacity-100' : '-translate-y-20 opacity-0 pointer-events-none'}`}>
          <div className="w-10 h-10 bg-yellow-500 rounded-full flex items-center justify-center shrink-0 animate-pulse"><Bell className="text-black" size={20} /></div>
          <div className="flex-1"><h4 className="text-white font-bold text-sm">{toast.title}</h4><p className="text-zinc-400 text-xs leading-tight mt-0.5">{toast.msg}</p></div>
      ...
      <div className="min-h-screen bg-zinc-950 text-white font-sans pb-24 w-full" style={{ selectionBackgroundColor: layoutConfig.color } as any}>
        <header className="fixed top-0 w-full z-40 bg-zinc-950/80 backdrop-blur-md border-b border-white/5 px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            {view !== 'LOBBY' ? <button onClick={handleBackToLobby} className="p-2 -ml-2 text-zinc-400 hover:text-white"><ChevronLeft size={28} /></button> : <div className="h-10 flex items-center justify-center">{layoutConfig.logo ? <img src={layoutConfig.logo} className="h-8 w-auto object-contain" /> : <div className="w-10 h-10 rounded-xl flex items-center justify-center shadow-lg" style={{ backgroundColor: layoutConfig.color }}><Zap className="text-black" size={20} /></div>}</div>}
            <div className="flex flex-col cursor-pointer" onClick={handleOpenDeposit}><span className="text-xs text-zinc-400 font-medium">{user ? 'Saldo' : 'Login'}</span><span className="text-sm font-bold text-white flex items-center gap-1">{user ? formatCurrency(balance) : 'Entrar'} <PlusCircle size={14} style={{ color: layoutConfig.color }} /></span></div>
          </div>
          <div className="flex items-center gap-3">
          ...
          <main className="flex flex-col items-center px-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="relative w-full max-w-sm bg-zinc-900 rounded-3xl p-1 shadow-2xl border border-zinc-800 overflow-hidden">
              ...
              <div className="relative bg-zinc-950 rounded-[20px] p-4 border border-zinc-800/50">
                {loading || prizesGrid.length === 0 ? (<div className="w-full aspect-square flex flex-col items-center justify-center gap-4 text-zinc-500"><div className="animate-spin rounded-full h-12 w-12 border-t-2" style={{ borderColor: layoutConfig.color }}></div><span className="text-xs font-medium uppercase tracking-widest">Carregando...</span></div>) : (<div className="flex flex-col gap-4"><div className="relative w-full aspect-square rounded-xl overflow-hidden shadow-lg border-2 border-zinc-800 group"><div className="absolute inset-0 grid grid-cols-3 grid-rows-3 gap-2 p-2 bg-zinc-900">{prizesGrid.map((prize, index) => (<div key={index} className={`rounded-lg flex flex-col items-center justify-center border transition-all duration-500 overflow-hidden relative ${winningIndices.includes(index) ? 'border-white z-10 scale-105 shadow-lg' : 'bg-white border-zinc-300'}`} style={winningIndices.includes(index) ? { backgroundColor: layoutConfig.color, boxShadow: `0 0 20px ${layoutConfig.color}99` } : {}}>{prize.image ? <img src={prize.image} className="w-[80%] h-[80%] object-contain drop-shadow-sm" /> : <span className={`font-black text-center leading-tight select-none p-1 ${winningIndices.includes(index) ? 'text-black text-xs' : 'text-zinc-900 text-[10px]'}`}>{prize.name}</span>}</div>))}</div><ScratchCard key={gameId} isRevealed={isGameFinished} onReveal={handleGameFinish} coverImage={layoutConfig.scratchCover} /></div>{!isGameFinished ? <button onClick={handleGameFinish} className="w-full bg-zinc-800 hover:bg-zinc-700 font-bold py-3.5 rounded-xl border border-zinc-700 flex items-center justify-center gap-2 transition-all active:scale-95 shadow-lg" style={{ color: layoutConfig.color }}><Zap size={18} className="fill-current" /> <span className="text-sm tracking-wide">REVELAR TUDO</span></button> : <button onClick={() => playRound()} className="w-full hover:opacity-90 text-black font-black py-3.5 rounded-xl flex items-center justify-center gap-2 transition-all active:scale-95 animate-pulse" style={{ backgroundColor: layoutConfig.color }}><RotateCw size={18} className="fill-current" /> <span className="text-sm tracking-wide">COMPRAR NOVA ({formatCurrency(activeGame.price)})</span></button>}</div>)}
              </div>
            </div>
            <div className="w-full max-w-sm mt-8">
              ...
              <div className="grid grid-cols-3 gap-3">{activeGame.prizes && activeGame.prizes.map((p: any, i: number) => (<div key={i} className="bg-zinc-900 p-2 rounded-xl border border-zinc-800 flex flex-col items-center justify-center gap-1 min-h-[80px]">{p.image ? <img src={p.image} className="h-8 w-8 object-contain mb-1" /> : null}<span className="font-bold text-[10px] text-center leading-tight text-white">{p.name}</span><span className="text-[10px] text-zinc-500 uppercase font-bold" style={{ color: layoutConfig.color }}>{formatCurrency(p.value)}</span></div>))}</div>
            </div>
          </main>
        )}
        ...
        {view === 'LOBBY' && (
          <main className="px-4 pb-8 md:max-w-7xl md:mx-auto">
            <div className="w-full rounded-2xl relative overflow-hidden shadow-lg border border-zinc-800 mb-8 group bg-zinc-900">{layoutConfig.banner ? <img src={layoutConfig.banner} className="w-full h-auto object-contain block" /> : <div className="h-52 bg-zinc-800 flex items-center justify-center font-bold text-zinc-600">Sem Banner</div>}</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">{gamesList.length > 0 ? (gamesList.map((game) => { const maxPrize = Math.max(...(game.prizes?.map(p => Number(p.value) || 0) || [0])); return (<div key={game.id} className="bg-zinc-900 rounded-xl overflow-hidden border border-zinc-800 shadow-lg hover:border-zinc-700 transition-all"><div className="w-full h-44 bg-zinc-950 relative flex items-center justify-center overflow-hidden">{game.cover ? <img src={game.cover} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-zinc-600 text-xs">Sem Imagem</div>}</div><div className="p-4 flex flex-col gap-1 items-start text-left"><h3 className="text-white font-bold text-lg leading-tight">{game.name}</h3><span className="text-[10px] font-bold uppercase tracking-wider mb-1" style={{ color: layoutConfig.color }}>PRÊMIOS DE ATÉ {formatCurrency(maxPrize)}</span><div className="w-full flex items-center justify-between mt-auto"><button onClick={() => handleOpenGame(game)} className="flex items-center gap-2 px-4 py-2 rounded-lg transition-transform active:scale-95 hover:brightness-110" style={{ backgroundColor: layoutConfig.color }}><div className="w-4 h-4 rounded-full bg-black/20 flex items-center justify-center"><Zap size={10} className="text-black fill-current" /></div><span className="text-black font-black text-sm uppercase">JOGAR</span><div className="bg-black/20 px-1.5 py-0.5 rounded text-[10px] font-bold text-black">{formatCurrency(game.price)}</div></button><button onClick={(e) => { e.stopPropagation(); setPreviewGame(game); }} className="flex items-center gap-1 text-[10px] font-bold text-zinc-400 hover:text-white"><Gift size={12} /> VER PRÊMIOS <ChevronRight size={10} /></button></div></div></div>); })) : <div className="text-center py-10 text-white font-bold text-lg animate-pulse col-span-full">Carregando jogos...</div>}</div>
          </main>
        )}
        ...
        <div className="fixed bottom-0 w-full bg-zinc-950/95 backdrop-blur-md border-t border-zinc-800 pb-2 pt-2 px-0 z-50 h-[80px] grid grid-cols-5 items-center">
          ...
          {deferredPrompt ? (<button onClick={handleInstallApp} className="flex flex-col items-center justify-center gap-1 h-full text-zinc-500 animate-pulse hover:text-green-500"><Download size={24} /> <span className="text-[10px] font-medium">Baixar App</span></button>) : (<button onClick={handleGoToWinners} className={`flex flex-col items-center justify-center gap-1 h-full ${view === 'WINNERS' ? 'text-white' : 'text-zinc-500'}`}><Trophy size={24} /> <span className="text-[10px] font-medium">Ganhadores</span></button>)}
      ...
      {previewGame && (<div className="fixed inset-0 z-[60] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={() => setPreviewGame(null)}><div className="w-full max-w-sm rounded-3xl p-1 relative shadow-2xl bg-gradient-to-br from-[#ffc700] to-yellow-700" onClick={e => e.stopPropagation()}><div className="bg-zinc-950 w-full h-full rounded-[20px] p-6 relative"><button onClick={() => setPreviewGame(null)} className="absolute top-4 right-4 text-zinc-500 hover:text-white"><X size={20}/></button><div className="text-center mb-6"><h3 className="text-xl font-black text-white mb-1 uppercase italic">{previewGame.name}</h3><p className="text-zinc-400 text-xs">Tabela de premiação</p></div><div className="grid grid-cols-3 gap-3 max-h-[60vh] overflow-y-auto">{previewGame.prizes && previewGame.prizes.map((p: any, i: number) => (<div key={i} className="bg-zinc-900 p-3 rounded-xl border border-zinc-800 flex flex-col items-center justify-center hover:bg-zinc-800">{p.image ? <img src={p.image} className="h-8 w-8 object-contain mb-1" /> : null}<span className="text-[10px] text-zinc-400 font-bold mb-1 text-center">{p.name}</span><span className="text-xs font-black text-yellow-500">{formatCurrency(p.value)}</span></div>))}</div><button onClick={() => { setPreviewGame(null); handleOpenGame(previewGame); }} className="w-full mt-6 bg-yellow-500 hover:bg-yellow-400 text-black font-black py-4 rounded-xl flex items-center justify-center gap-2 uppercase tracking-wide shadow-lg" style={{ backgroundColor: layoutConfig.color }}>JOGAR AGORA ({formatCurrency(previewGame.price)})</button></div></div></div>)}
      {showMysteryBox && (<div className="fixed inset-0 z-[60] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in" onClick={() => setShowMysteryBox(false)}><div className="w-full max-w-sm rounded-3xl p-1 relative shadow-2xl bg-gradient-to-br from-purple-500 to-indigo-600" onClick={e => e.stopPropagation()}><div className="bg-zinc-950 w-full h-full rounded-[20px] p-8 relative text-center"><button onClick={() => setShowMysteryBox(false)} className="absolute top-4 right-4 text-zinc-500 hover:text-white"><X size={20}/></button><div className="w-24 h-24 bg-purple-500/20 rounded-full flex items-center justify-center mx-auto mb-6 border-4 border-purple-500 animate-bounce"><Gift size={48} className="text-purple-400" /></div><h3 className="text-2xl font-black text-white mb-2 uppercase italic">Bônus Diário</h3>{dailyGiftConfig.active ? (bonusAvailable ? (<><p className="text-zinc-400 text-sm mb-6">Ganhe <span className="text-purple-400 font-bold">{formatCurrency(dailyGiftConfig.amount)} de Saldo</span> para jogar onde quiser!</p><button onClick={claimDailyBonus} disabled={claimingBonus} className="w-full bg-purple-600 hover:bg-purple-500 text-white font-bold py-4 rounded-xl shadow-lg transition-transform active:scale-95 flex items-center justify-center gap-2">{claimingBonus ? 'Resgatando...' : 'RESGATAR AGORA'}</button></>) : (<><p className="text-zinc-400 text-sm mb-2">Volte amanhã para mais.</p><div className="bg-zinc-900 p-4 rounded-xl border border-zinc-800 mb-6"><p className="text-xs text-zinc-500 uppercase font-bold mb-1">Próximo bônus em</p><div className="text-2xl font-black text-white flex items-center justify-center gap-2"><Clock size={20} className="text-purple-500" /> {timeLeft}</div></div></>)) : (<p className="text-zinc-500 text-sm py-4">Nenhuma campanha de bônus ativa no momento.</p>)}</div></div></div>)}
      {showPopup && (<div className="fixed inset-0 bg-black/90 backdrop-blur-sm flex items-center justify-center z-[70] p-6 animate-in fade-in"><div className="w-full max-w-sm bg-zinc-900 rounded-3xl p-6 border border-zinc-800 text-center relative shadow-2xl"><div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4 border-4 ${resultType === 'WIN' ? 'text-black shadow-lg' : 'bg-zinc-800 border-zinc-700 text-zinc-500'}`} style={resultType === 'WIN' ? { backgroundColor: layoutConfig.color, borderColor: '#fff' } : {}}>{resultType === 'WIN' ? <Trophy size={40} className="fill-current" /> : <XCircle size={40} />}</div><h2 className="text-2xl font-black text-white mb-2 uppercase italic">{resultType === 'WIN' ? 'Parabéns!' : 'Não foi dessa vez'}</h2>{resultType === 'WIN' ? <div className="p-4 rounded-2xl border mb-6" style={{ backgroundColor: `${layoutConfig.color}1a`, borderColor: `${layoutConfig.color}33` }}><p className="text-xs font-bold uppercase tracking-wider mb-1" style={{ color: layoutConfig.color }}>Você ganhou</p><p className="text-4xl font-black tracking-tighter" style={{ color: layoutConfig.color }}>{winAmount}</p></div> : <p className="text-zinc-400 text-sm mb-8 leading-relaxed">Tente novamente.</p>}<button onClick={() => playRound()} className="w-full text-black font-black py-4 rounded-xl text-lg flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-transform mb-3" style={{ backgroundColor: layoutConfig.color }}><RotateCw size={20} strokeWidth={3} /> JOGAR NOVAMENTE</button><button onClick={handleBackToLobby} className="text-zinc-500 font-bold text-sm hover:text-white py-2">Voltar ao Início</button></div></div>)}
