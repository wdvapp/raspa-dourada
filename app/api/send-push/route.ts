...
// ... (Configuração do Admin continua igual)
...
export async function POST(request: Request) {
  try {
    // AGORA ACEITAMOS A IMAGEM (BANNER) DE NOVO
    ...
    if (tokensToSend.length > 0 && admin.apps.length) {
        ...
        const message = {
            data: {
                title: title,
                body: body,
                image: image || "", // <--- MANDANDO A IMAGEM AQUI
                link: link || "/"
            },
            tokens: tokensToSend,
        };
        ...
        try {
            ...
            console.log(`Push com Banner enviado: ${successCount}`);
        } catch (pushError) {
