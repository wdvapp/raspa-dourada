...
export default function DepositModal({ isOpen, onClose, userEmail, userId }: DepositModalProps) {
  ...
  // Detector de Pagamento Automático
  useEffect(() => {
    if (isOpen && step === 2 && depositData?.txid) {
        // CORREÇÃO: A API da Pixup retorna 'txid', então monitoramos pelo ID do documento ou buscamos pelo txid
        // Se a API retorna o ID do documento firestore como 'id' (no JSON), usamos ele:
        const docId = depositData.id || depositData.txid; 
        
        // Se for o ID do documento direto:
        if(depositData.id) {
  ...
  const handleGeneratePix = async () => {
    ...
    try {
      // --- AQUI ESTAVA O ERRO ---
      // Sua pasta chama 'deposit', não 'pixup'.
      ...
      if (!response.ok) {
        ...
        // Se der erro 405, é porque o caminho ainda está errado.
        throw new Error("Falha na comunicação com o servidor.");
      }

      const data = await response.json();

      // Suporte para resposta da Pixup (qrcode_image, qrcode_text) OU MercadoPago (qrcode, copypaste)
      // Assim funciona com qualquer um dos dois códigos de API que estiver lá.
      const qrCodeBase64 = data.qrcode || data.qrcode_image; // Tenta os dois
      const copyPasteCode = data.copypaste || data.qrcode_text || data.pixCopiaECola; // Tenta os três

      if (qrCodeBase64 && (data.id || data.txid)) {
        setDepositData({
            ...data,
            qrcode: qrCodeBase64,
        ...
        alert('Erro: A API não retornou o QR Code. Verifique o console.');
      }
    } catch (error) {
      console.error(error);
        ...
        {step === 2 && depositData && (
          <div className="p-8 text-center">
            ...
            <div className="bg-white p-4 rounded-2xl mx-auto w-64 h-64 mb-6 shadow-xl flex items-center justify-center">
               {/* Renderiza imagem base64 ou URL dependendo do que vier */}
               {depositData.qrcode ? (
                   depositData.qrcode.startsWith('http') ? 
                   <img src={depositData.qrcode} alt="QR Code" className="w-full h-full object-contain" /> :
                   <img src={`data:image/png;base64,${depositData.qrcode}`} alt="QR Code" className="w-full h-full object-contain" />
               ) : <div className="text-black text-sm">Erro ao exibir QR</div>}
            </div>
