...
export default function DepositModal({ isOpen, onClose, userEmail, userId }: DepositModalProps) {
  ...
  // --- ESCUTA AUTOMÁTICA DO PAGAMENTO (AJUSTADA PARA PIXUP) ---
  useEffect(() => {
    if (isOpen && step === 2 && depositData?.txid) {
        // A Pixup usa 'txid'. Vamos buscar no firebase pelo txid.
        const q = query(collection(db, 'deposits'), where('txid', '==', depositData.txid));
        
        const unsubscribe = onSnapshot(q, (snapshot) => {
            snapshot.docChanges().forEach((change) => {
                if (change.type === 'modified' || change.type === 'added') {
                    const data = change.doc.data();
                    // Se o status mudar para pago, fecha
  ...
  const handleGeneratePix = async () => {
    ...
    try {
      ...
      // AQUI ESTAVA O ERRO DE COMUNICAÇÃO
      // Sua API retorna: qrcode_image, qrcode_text, txid
      if (data.qrcode_text && data.txid) {
        setDepositData(data);
        setStep(2);
      } else {
        ...
        {step === 2 && depositData && (
          <div className="p-8 text-center">
            ...
            <div className="bg-white p-4 rounded-2xl mx-auto w-64 h-64 mb-6 shadow-xl flex items-center justify-center">
               {/* AJUSTE PARA A SUA API: Usa qrcode_image direto */}
               {depositData.qrcode_image ? <img src={depositData.qrcode_image} alt="QR Code Pix" className="w-full h-full object-contain" /> : <div className="text-black text-sm">QR Code indisponível</div>}
            </div>
            
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-1 pl-4 flex items-center gap-2 mb-6">
                {/* AJUSTE PARA A SUA API: Usa qrcode_text */}
                <input readOnly value={depositData.qrcode_text || ''} className="bg-transparent border-none text-zinc-500 text-xs w-full focus:outline-none truncate" />
