...
// Renomeei User para UserIcon para não dar conflito com a variável user
...
// ESTA INTERFACE AGORA BATE EXATAMENTE COM O SEU PAGE.TSX
interface ProfileSidebarProps {
    isOpen: boolean;
    onClose: () => void;
    user: any;
    balance: number;      // Aceita o saldo
    onLogout: () => void; // Aceita a função de sair
}
...
export default function ProfileSidebar({ isOpen, onClose, user, balance, onLogout }: ProfileSidebarProps) {
    ...
    const handleWithdraw = async () => {
        ...
        try {
            ...
            const msg = typeof error === 'string' ? error : 'Erro ao processar.';
                        ...
                        <div className="space-y-5">
                            <div className="bg-yellow-500/10 p-4 rounded-xl border border-yellow-500/20 flex gap-3">
                                ...
                                <p className="text-xs text-yellow-200/80 leading-relaxed">
                                    O valor é descontado imediatamente do saldo.
                                </p>
                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <input type="text" value={pixKey} onChange={e => setPixKey(e.target.value)} placeholder="Sua chave..." className="w-full bg-zinc-950 border border-zinc-800 rounded-xl p-4 text-white focus:border-[#ffc700] focus:outline-none transition-colors text-sm" />
                                </div>

                                <div>
                                    <label className="text-xs text-zinc-500 font-bold uppercase mb-2 block">Valor (R$)</label>
                                    <input type="tel" inputMode="numeric" value={formatDisplayAmount(withdrawAmount)} onChange={handleAmountChange} placeholder="R$ 0,00" className="w-full bg-zinc-950 border border-zinc-800 rounded-xl p-4 text-white focus:border-[#ffc700] focus:outline-none transition-colors text-xl font-bold" />
                                </div>
                                ...
                                <button disabled={loadingWithdraw} onClick={handleWithdraw} className="w-full bg-[#ffc700] hover:bg-[#e6b300] disabled:opacity-50 text-black font-black py-4 rounded-xl text-sm shadow-lg flex items-center justify-center gap-2 transition-all active:scale-95 uppercase">
                                    {loadingWithdraw ? <Loader2 className="animate-spin" size={18} /> : <PiggyBank size={18} />} CONFIRMAR SAQUE
                                </button>
                            </div>
                        </div>
                    )}
                                ...
                                withdrawHistory.map(item => (
                                    <div key={item.id} className="bg-zinc-900 p-4 rounded-xl border border-zinc-800 flex justify-between items-center">
                                        <div>
                                            ...
                                            <span className="text-[10px] text-zinc-500 uppercase">{new Date(item.createdAt).toLocaleDateString('pt-BR')}</span>
                                        </div>
                                        {getStatusBadge(item.status)}
                                    </div>
                                ))
                            ) : (
                                <div className="text-center py-10 text-zinc-600 text-xs font-medium border-2 border-dashed border-zinc-800 rounded-xl">Sem histórico.</div>
                            )}
                        </div>
                    )}
                </div>
